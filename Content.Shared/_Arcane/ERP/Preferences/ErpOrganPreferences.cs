...
/// <summary>
/// Per-character organ appearance configuration stored on <see cref="HumanoidCharacterProfile"/>.
/// </summary>
[DataDefinition]
[Serializable, NetSerializable]
public sealed partial class ErpOrganPreferences
{
    /// <summary>Organ configs keyed by organ slot id ("penis", "vagina", "breasts", …).</summary>
    [DataField]
    public Dictionary<string, ErpOrganConfig> Organs { get; set; } = new();

    /// <summary>Breast jiggle while walking / stopping.</summary>
    [DataField]
    ...
    public ErpOrganPreferences Clone()
    {
        ...
        {
            BreastBounce = BreastBounce.Clone(),
        };
    ...
    public bool MemberwiseEquals(ErpOrganPreferences? other)
    {
        if (other == null)
            return false;

        if (!BreastBounce.MemberwiseEquals(other.BreastBounce))
            return false;

        if (Organs.Count != other.Organs.Count)
            return false;

        foreach (var (slot, cfg) in Organs)
        {
            if (!other.Organs.TryGetValue(slot, out var otherCfg))
                return false;

            if (cfg.Variant != otherCfg.Variant ||
                cfg.Size != otherCfg.Size ||
                cfg.Color != otherCfg.Color)
                return false;
        }
...
[DataDefinition]
[Serializable, NetSerializable]
public sealed partial class ErpOrganConfig
{
...
public static class ErpOrganSlots
{
    ...
    /// <summary>
    /// Fallback variant list per slot, used only when the species entity cannot be resolved.
    /// </summary>
