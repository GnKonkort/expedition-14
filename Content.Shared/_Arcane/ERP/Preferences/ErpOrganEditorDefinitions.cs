...
public static class ErpOrganEditorDefinitions
{
        ...
        IComponentFactory componentFactory)
    {
        ...
        if (!entityPrototype.TryGetComponent(out EroticOrgansComponent? organs, componentFactory))
        ...
        foreach (var entry in GetEntries(organs, sex))
        {
            if (string.IsNullOrEmpty(entry.Slot) ||
                !prototype.TryIndex(entry.Proto, out var organPrototype) ||
                !organPrototype.TryGetComponent(out EroticOrganComponent? organ, componentFactory) ||
                !organ.EditorVisible)
            {
                continue;
            }
        ...
        var definitions = result.Values.ToList();
